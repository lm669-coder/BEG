...
def get_decomptes_totals(id_chantier):
    conn = get_conn()
    row = conn.execute(
...
def get_montant_facture(id_chantier):
    conn = get_conn()
    row = conn.execute(
...
def bilan_exists_for_chantier(id_chantier):
    conn = get_conn()
...
def save_bilan(data):
    ...
    for table in ["bilan_parties_prenantes", "bilan_sous_traitants",
                  "bilan_postes_perte", "bilan_postes_surbenefice", "bilan_travaux_internes"]:
