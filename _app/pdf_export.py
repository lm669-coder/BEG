...
STYLES = {
    "doc_title": _s("doc_title", fontSize=20, fontName="Helvetica-Bold",
                    textColor=C_DARK, alignment=TA_CENTER, leading=24),
    "doc_sub":   _s("doc_sub",   fontSize=9,  textColor=C_GRAY, alignment=TA_CENTER),
    "sec":       _s("sec",       fontSize=12, fontName="Helvetica-Bold",
                    textColor=C_MID, leading=15, spaceBefore=2),
    ...
    "note":      _s("note",      fontSize=8,   fontName="Helvetica-Oblique",
...
def _data_table(header_row, data_rows, col_widths, row_bg=None, last_row_bold=False, extra_style=None):
...
def generate_pdf(bilan: dict, chantier: dict, montant_facture: float) -> bytes:
    ...
    story.append(_kv_table([
        ...
        ("Écart (+ = avance / − = retard)", f"{respect:+d} {unite}" if respect is not None else "—"),
    ]))
        ...
        if travaux:
            ...
            for t in travaux:
                ...
                rows.append([
                    ...
                    Paragraph(f"{coeff:.3f}" if coeff is not None else "—", STYLES["value"]),
                ])
            ...
            rows.append([
                ...
                Paragraph(f"{coeff_t:.3f}" if coeff_t is not None else "—", STYLES["value_b"]),
            ])
            cw = [CONTENT_W * 0.46, CONTENT_W * 0.18, CONTENT_W * 0.18, CONTENT_W * 0.18]
    ...
    if sts:
        ...
        header = ([Paragraph("Sous-traitant", STYLES["th"])] +
                  [Paragraph(c, STYLES["th"]) for c in criteria_labels] +
                  [Paragraph("Moy.", STYLES["th"])])
        rows = []
        avg_totals = []
        for st in sts:
            scores = [st.get(k) for k in
                      ("respect_prix", "respect_delais", "respect_securite",
        ...
        tbl = _data_table(header, rows, cw, last_row_bold=bool(avg_totals),
                          extra_style=[("ALIGN", (1, 0), (-1, -1), "CENTER")])
        story.append(tbl)

        if bilan.get("notes_sous_traitants"):
            story.append(Spacer(1, 0.15 * cm))
            story.append(Paragraph(f"Notes : {bilan['notes_sous_traitants']}", STYLES["note"]))
