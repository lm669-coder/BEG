...
ST_FIELDS = ["nom", "respect_prix", "respect_delais", "respect_securite",
             "respect_qualite", "reactivite", "communication"]
...
with st.sidebar:
    ...
    page = st.radio(
        "Navigation",
        ["📝 Nouveau / Modifier bilan", "📋 Historique bilans", "📊 Dashboard", "⬆ Importer données Excel"],
...
def _apply_bilan_to_state(bilan):
    ...
    pp_fixed = [p for p in bilan["parties_prenantes"]
                if p["role"] in ("Client", "Architecte", "Bureau d'étude")]
    pp_extra = [p for p in bilan["parties_prenantes"]
                if p["role"] not in ("Client", "Architecte", "Bureau d'étude")]
    for r, rkey in [("Client", "client"), ("Architecte", "architecte"), ("Bureau d'étude", "bureau_d__tude")]:
...
if page == "📝 Nouveau / Modifier bilan":
    ...
    with col_load2:
        ...
        if st.button("🔄 Nouveau bilan vierge"):
            for k in list(st.session_state.keys()):
                if k.startswith(("f_", "n_", "st_", "pp_", "pp2_", "perte_", "surb_", "trav_")):
                    del st.session_state[k]
    ...
    st.markdown('<div class="section-header">1. IDENTIFICATION</div>', unsafe_allow_html=True)

    col1, col2 = st.columns([2, 3])
    with col1:
        id_chantier = st.text_input("ID Chantier *", key="f_id_chantier",
    ...
    if chantier_info:
        ...
        st.warning("⚠ ID chantier non trouvé dans la base. Vérifiez ou importez les données.")
    ...
    st.markdown('<div class="section-header">2. PARTIES PRENANTES</div>', unsafe_allow_html=True)

    for role_label, rkey in [("Client", "client"), ("Architecte", "architecte"), ("Bureau d'étude", "bureau_d__tude")]:
        st.markdown(f'<div class="sub-header">{role_label}</div>', unsafe_allow_html=True)
        c1, c2, c3 = st.columns([3, 4, 1])
        c1.text_input("Nom", key=f"pp_{rkey}_nom")
        c2.text_input("Relation / Appréciation", key=f"pp_{rkey}_relation")
        c3.number_input("Éval. /5", min_value=1, max_value=5, step=1, key=f"pp_{rkey}_eval")

    n_pp2 = _get("n_pp2", 0)
    if n_pp2 > 0:
        st.markdown('<div class="sub-header">Autres parties prenantes</div>', unsafe_allow_html=True)
        for i in range(n_pp2):
            ...
            c3.number_input("Éval.", min_value=1, max_value=5, step=1, key=f"pp2_{i}_evaluation")
            with c4:
                st.write("")
    ...
    st.markdown('<div class="section-header">3. PERFORMANCE</div>', unsafe_allow_html=True)

    st.markdown('<div class="sub-header">Délais</div>', unsafe_allow_html=True)
    for _k in ("f_delai_soumission", "f_delai_contractuel", "f_delai_complementaire", "f_delai_reel"):
    ...
    ds = c1.number_input("Délai soumission", min_value=0, step=1, key="f_delai_soumission")
    dc = c2.number_input("Délai contractuel", min_value=0, step=1, key="f_delai_contractuel")
    dcomp = c3.number_input("Délai complémentaire", min_value=0, step=1, key="f_delai_complementaire")
    ...
    for _k in ("f_montant_base_pv", "f_montant_decomptes_pv", "f_prix_de_revient", "f_marge_devis", "f_marge_finale"):
        v = st.session_state.get(_k)
    ...
    c1, c2, c3, c4 = st.columns(4)
    mb = c1.number_input("Montant base PV (€ HTVA)", min_value=0.0, step=1000.0,
                         format="%.2f", key="f_montant_base_pv")
    md = c2.number_input("Montant décomptes PV (€)", min_value=0.0, step=1000.0,
                         format="%.2f", key="f_montant_decomptes_pv")
    with c3:
        st.metric("Total PV", f"{mb + md:,.0f} €".replace(",", " "))
    with c4:
        st.metric("Total facturé (EA)", f"{montant_facture:,.0f} €".replace(",", " "),
                  help="Calculé automatiquement depuis les états d'avancement importés")

    st.markdown('<div class="sub-header">Prix de Revient (PR)</div>', unsafe_allow_html=True)
    pr_val = st.number_input("PR Soumission (€ HTVA)", min_value=0.0, step=1000.0,
                              format="%.2f", key="f_prix_de_revient")
    pv_total = mb + md

    st.markdown('<div class="sub-header">Marges</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    c1.number_input("Marge devis (%)", min_value=-100.0, max_value=100.0,
                    step=0.1, format="%.2f", key="f_marge_devis")
    c2.number_input("Marge finale d'exécution (%)", min_value=-100.0, max_value=100.0,
                    step=0.1, format="%.2f", key="f_marge_finale")

    # ══ SECTION RP/RD ════════════════════════════════════════════════════
    st.markdown('<div class="section-header">RETENUES (RP)</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header">4. RENDEMENTS & EXÉCUTION</div>', unsafe_allow_html=True)
    ...
    def render_postes(cat, label, color):
        st.markdown(f'<div class="sub-header" style="color:{color}">{label}</div>', unsafe_allow_html=True)
        n = _get(f"n_{cat}", 0)
        for i in range(n):
            ...
            c2.number_input("PRS (€)", min_value=0.0, step=100.0, format="%.2f",
                            key=f"{cat}_{i}_prs")
            c3.number_input("PRE (€)", min_value=0.0, step=100.0, format="%.2f",
                            key=f"{cat}_{i}_pre")
            ...
            if st.button("✕ Supprimer", key=f"del_{cat}_{i}"):
                ...
                st.rerun()
            st.divider()
        if st.button(f"+ Ajouter poste", key=f"add_{cat}"):
    ...
    st.markdown('<div class="sub-header">Travaux réalisés en interne</div>', unsafe_allow_html=True)
    ...
    for i in range(n_trav):
        ...
        hs = c2.number_input("H-S (budg.)", min_value=0.0, step=0.5, key=f"trav_{i}_heures_soumission")
        he = c3.number_input("H-E (réel)", min_value=0.0, step=0.5, key=f"trav_{i}_heures_execution")
    ...
    if n_trav > 0:
        ...
        c3.metric("Coefficient global", f"{coeff_total:.3f}" if coeff_total is not None else "—")

    if st.button("+ Ajouter travaux internes"):
        _add_item("trav", TRAVAIL_FIELDS, defaults={"heures_soumission": 0.0, "heures_execution": 0.0})
        st.rerun()
    ...
    c2.number_input("Satisfaction client /5", min_value=1, max_value=5, step=1,
                    key="f_satisfaction_client")
    st.text_area("Travaux non satisfaisants (description + responsable)", key="f_travaux_non_satisfaisants", height=80)
    ...
    accident = st.radio("Accident(s) sur chantier ?", ["Non", "Oui"], horizontal=True, key="f_accidents")
    if accident == "Oui":
        st.text_area("Description des accidents (circonstances, propre personnel / sous-traitant)",
                     key="f_desc_accidents", height=100)
    st.text_area("Améliorations / mesures préventives", key="f_ameliorations_securite", height=80)

    # ══ SECTION 7 — Sous-traitants ═══════════════════════════════════════
    st.markdown('<div class="section-header">7. SOUS-TRAITANTS</div>', unsafe_allow_html=True)
    st.caption("Évaluation de 1 (très mauvais) à 5 (excellent)")
    n_st = _get("n_st_", 0)
    criteria_labels = ["Prix", "Délais", "Sécu.", "Qualité", "Réact.", "Comm."]
    criteria_keys = ["respect_prix", "respect_delais", "respect_securite",
                     "respect_qualite", "reactivite", "communication"]
    ...
    for i in range(n_st):
        ...
        for j, (lbl, kk) in enumerate(zip(criteria_labels, criteria_keys)):
            v = cols[j + 1].number_input(lbl, min_value=1, max_value=5, step=1,
                                          key=f"st__{i}_{kk}")
            scores.append(v)
    ...
    st.markdown('<div class="section-header">8. COMMENTAIRE GÉNÉRAL / SYNTHÈSE</div>', unsafe_allow_html=True)
    st.text_area("Points forts, points faibles, enseignements pour l'avenir",
                 key="f_commentaire_general", height=150)
    ...
    def _build_data():
        ...
        return {
            ...
            "postes_surbenefice": _collect_list("surb", _get("n_surb", 0), POSTE_FIELDS),
            "travaux_internes": _collect_list("trav", _get("n_trav", 0), TRAVAIL_FIELDS),
        }
    ...
    with col_save2:
        if st.button("📄 Générer PDF", use_container_width=True):
            if not _get("f_id_chantier"):
                ...
                data = _build_data()
                ch = chantier_info or {"intitule": "", "gestionnaire": "", "client": "",
                                       "secteur": "", "province": ""}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not bilans:
        ...
        gestionnaires = ["Tous"] + sorted({b.get("gestionnaire") or "?" for b in bilans if b.get("gestionnaire")})

        col_f1, col_f2, col_f3 = st.columns(3)
        search = col_f1.text_input("🔍 Recherche (ID, nom, client)", "")
        gest_filter = col_f2.selectbox("Gestionnaire", gestionnaires)
        sort_by = col_f3.selectbox("Trier par", ["Date (récent)", "ID chantier", "Client"])

        rows = bilans
        if search:
            s = search.lower()
            rows = [b for b in rows if
                    s in str(b.get("id_chantier") or "").lower() or
                    s in (b.get("intitule") or "").lower() or
                    s in (b.get("client") or "").lower()]
        if gest_filter != "Tous":
        ...
        labels = [f"{b['id_chantier']} — {b.get('intitule') or 'Sans nom'} ({b.get('date_bilan') or '?'})"
                  for b in rows]
        ...
        if sel_id != "—":
            ...
            if _get("_confirm_delete_id") == target_id:
                st.warning(f"⚠ Supprimer définitivement le bilan **{sel_id}** ? Cette action est irréversible.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not bilans:
        ...
        c2.metric("Satisfaction client moy.", f"{avg_sat:.1f}/5" if not pd.isna(avg_sat) else "—")
        avg_marge = df["marge_finale"].dropna().mean()
        c3.metric("Marge finale moy.", f"{avg_marge:.1f}%" if not pd.isna(avg_marge) else "—")
        accidents = df[df.get("accidents_chantier") is not None].shape[0] if "accidents_chantier" in df else 0
        c4.metric("Chantiers / bilans", f"{len(db.get_all_chantiers())} / {len(bilans)}")
        ...
        with col_g1:
            ...
            sat_df = df[df["satisfaction_client"].notna()][["id_chantier", "satisfaction_client"]]
            if not sat_df.empty:
        ...
        st.dataframe(df[["id_chantier", "intitule", "gestionnaire", "client",
                          "date_bilan", "marge_finale", "satisfaction_client"]].rename(columns={
            "id_chantier": "ID", "intitule": "Chantier", "gestionnaire": "Gest.",
            "client": "Client", "date_bilan": "Date", "marge_finale": "Marge %",
        ...
        chantiers = db.get_all_chantiers()
        if chantiers:
            st.dataframe(pd.DataFrame(chantiers), use_container_width=True, hide_index=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.caption("Fichier contenant la liste de vos chantiers avec ID, gestionnaire, client, montants...")
    ...
    if file_ch:
        ...
        if sheet_names_ch:
            ...
            sheet_ch = col_s1.selectbox("Feuille contenant les chantiers", sheet_names_ch,
                                         key="sheet_ch")
            header_row_ch = col_s2.number_input("Ligne d'en-têtes", min_value=1, max_value=10,
                                                  value=1, key="header_row_ch")
            first_data_ch = col_s3.number_input("Première ligne de données", min_value=2,
                                                  max_value=20, value=2, key="first_data_ch")

            # Aperçu
            try:
                file_ch.seek(0)
                preview = imp.get_sheet_preview(file_ch, sheet_ch, max_rows=int(header_row_ch) + 3)
                if preview:
                    st.caption("Aperçu (3 premières lignes de données) :")
                    headers = [str(c) if c is not None else "" for c in preview[int(header_row_ch) - 1]]
                    data_rows = preview[int(header_row_ch):]
                    if data_rows:
                        _df_prev = pd.DataFrame(data_rows, columns=headers if headers else None)
                        st.dataframe(_df_prev.head(3), use_container_width=True, hide_index=True)
            except Exception as ex:
                st.warning(f"Aperçu indisponible : {ex}")

            overwrite_ch = st.checkbox("Écraser les chantiers existants (même ID)",
                                        key="overwrite_ch")
                ...
                with st.spinner("Import en cours..."):
                    try:
                        ...
                        ok, skip, detected = imp.import_chantiers(
                            file_ch, sheet_ch, conn,
                            header_row=int(header_row_ch),
                            first_data_row=int(first_data_ch),
                            overwrite=overwrite_ch,
                        )
                        conn.close()
                        st.success(f"✅ {ok} chantier(s) importé(s), {skip} ignoré(s) (déjà présents).")
    ...
    st.caption("Fichier contenant les montants facturés par chantier — utilisé pour le calcul automatique du total facturé.")
    ...
    if file_ea:
        ...
        if sheet_names_ea:
            ...
            sheet_ea = col_e1.selectbox("Feuille contenant les EA", sheet_names_ea,
                                         key="sheet_ea")
            header_row_ea = col_e2.number_input("Ligne d'en-têtes", min_value=1, max_value=10,
                                                  value=1, key="header_row_ea")
            first_data_ea = col_e3.number_input("Première ligne de données", min_value=2,
                                                  max_value=20, value=2, key="first_data_ea")

            # Aperçu + sélection colonnes clés
            try:
                file_ea.seek(0)
                preview_ea = imp.get_sheet_preview(file_ea, sheet_ea,
                                                    max_rows=int(header_row_ea) + 3)
                if preview_ea:
                    headers_ea = [str(c) if c is not None else f"Col {i}"
                    ...
                    if data_rows_ea:
                        ...
                        st.dataframe(_df_ea.head(3), use_container_width=True, hide_index=True)

                    st.caption("Sélectionner manuellement les colonnes clés (si la détection auto échoue) :")
                    ...
                    col_id_idx = (headers_ea.index(col_id_sel) if col_id_sel != "Auto-détection"
                                  else None)
                    col_mt_idx = (headers_ea.index(col_mt_sel) if col_mt_sel != "Auto-détection"
                                  else None)
            except Exception as ex:
                st.warning(f"Aperçu indisponible : {ex}")
                col_id_idx = col_mt_idx = None

            overwrite_ea = st.checkbox("Vider et réimporter tous les EA (recommandé pour mise à jour)",
                                        key="overwrite_ea", value=True)
            ...
            if st.button("▶ Importer les EA", type="primary", key="btn_import_ea"):
                with st.spinner("Import en cours..."):
                    try:
                        ...
                        st.caption(f"Colonnes utilisées : ID chantier = col {det_ea['col_id']}, "
    ...
    st.caption("Fichier listant les décomptes par chantier — utilisé pour préremplir le délai complémentaire et le montant des décomptes dans le formulaire.")
    ...
    if file_dec:
        ...
        if sheet_names_dec:
            ...
            sheet_dec = col_d1.selectbox("Feuille contenant les décomptes", sheet_names_dec,
                                          key="sheet_dec")
            header_row_dec = col_d2.number_input("Ligne d'en-têtes", min_value=1, max_value=10,
                                                   value=1, key="header_row_dec")
            first_data_dec = col_d3.number_input("Première ligne de données", min_value=2,
                                                   max_value=20, value=2, key="first_data_dec")
            ...
            try:
                file_dec.seek(0)
                preview_dec = imp.get_sheet_preview(file_dec, sheet_dec,
                                                     max_rows=int(header_row_dec) + 3)
                if preview_dec:
                    headers_dec = [str(c) if c is not None else f"Col {i}"
                                   for i, c in enumerate(preview_dec[int(header_row_dec) - 1])]
                    st.caption("Aperçu :")
                    data_rows_dec = preview_dec[int(header_row_dec):]
                    if data_rows_dec:
                        st.dataframe(pd.DataFrame(data_rows_dec, columns=headers_dec).head(3),
                                     use_container_width=True, hide_index=True)
            except Exception as ex:
                st.warning(f"Aperçu indisponible : {ex}")

            if headers_dec:
                st.caption("Sélection manuelle des colonnes clés (laisser Auto-détection si possible) :")
                cm1, cm2, cm3, cm4 = st.columns(4)
                sel_dec_id = cm1.selectbox("Colonne : ID chantier",
                                           ["Auto-détection"] + headers_dec, key="dec_col_id")
                sel_dec_mt = cm2.selectbox("Colonne : Montant décompte",
                                           ["Auto-détection"] + headers_dec, key="dec_col_mt")
                sel_dec_dl = cm3.selectbox("Colonne : Délai complémentaire",
                                           ["Auto-détection"] + headers_dec, key="dec_col_dl")
                sel_dec_ap = cm4.selectbox("Colonne : Approuvé (0/1)",
                                           ["Auto-détection"] + headers_dec, key="dec_col_ap")

                dec_col_id = headers_dec.index(sel_dec_id) if sel_dec_id != "Auto-détection" else None
                dec_col_mt = headers_dec.index(sel_dec_mt) if sel_dec_mt != "Auto-détection" else None
                dec_col_dl = headers_dec.index(sel_dec_dl) if sel_dec_dl != "Auto-détection" else None
                dec_col_ap = headers_dec.index(sel_dec_ap) if sel_dec_ap != "Auto-détection" else None

            overwrite_dec = st.checkbox("Vider et réimporter tous les décomptes",
                                         key="overwrite_dec", value=True)

            if st.button("▶ Importer les décomptes", type="primary", key="btn_import_dec"):
                with st.spinner("Import en cours..."):
                    try:
                        file_dec.seek(0)
                        conn = db.get_conn()
                        ok_dec, det_dec = imp.import_decomptes(
                            file_dec, sheet_dec, conn,
                        ...
                        _tot = _c.execute("SELECT COUNT(*) FROM decomptes").fetchone()[0]
                        _app = _c.execute("SELECT COUNT(*) FROM decomptes WHERE statut_accepte=1").fetchone()[0]
                        _c.close()
                        st.success(f"✅ {ok_dec} ligne(s) importée(s) — {_app} approuvées / {_tot} total.")
                        if det_dec.get("col_accepte") is None:
                            st.warning("⚠ Colonne 'Approuvé' non détectée automatiquement. Sélectionnez-la manuellement.")
