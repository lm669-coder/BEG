...
def _safe(val):
    ...
    s = str(val).strip()
    return s if s and s.lower() not in ("none", "nan", "#n/a", "#ref!", "#value!") else None
...
# ── Import chantiers ──────────────────────────────────────────────────────────
def import_chantiers(source, sheet_name, conn,
    ...
    # Détection colonnes
    col = {
        "id":         _find_col(headers_norm, "id chantier", "num chantier", "reference", " id", "^id"),
    ...
    if col["pr"] is None:
        col["pr"] = next((i for i, h in enumerate(headers_norm) if h.strip() == "pr"), None)
...
# ── Import décomptes ─────────────────────────────────────────────────────────
def import_decomptes(source, sheet_name, conn,
                     header_row=1, first_data_row=2,
                     col_id=None, col_montant=None, col_delai=None,
                     col_num=None, col_intitule=None, col_accepte=None,
                     overwrite=False):
    wb = open_workbook(source)
    ws = wb[sheet_name]
    rows = list(ws.iter_rows(min_row=header_row, values_only=True))
    ...
    if col_id is None:
        ...
        if col_id is None:
            col_id = next((i for i, h in enumerate(headers_norm) if h.strip() in ("n° chantier", "num chantier", "chantier")), None)
        if col_id is None:
            col_id = 0
    ...
    if col_num is None:
        col_num = _find_col(headers_norm, "n° dc", "n dc", "num dc", "n° decompte", "n decompte")
        if col_num is None:
            col_num = _find_col(headers_norm, "denomination")
    if col_intitule is None:
        col_intitule = _find_col(headers_norm, "denomination", "intitul", "libelle", "objet")
    if col_accepte is None:
        col_accepte = _find_col(headers_norm, "approuve", "approuv", "accepte", "accept")
    ...
    for row in rows[first_data_row - header_row:]:
        ...
        conn.execute(
            ...
            "VALUES (?,?,?,?,?,?)",
            (id_c, _safe(g(row, col_num)), _safe(g(row, col_intitule)),
             _num(g(row, col_montant)), _num(g(row, col_delai)), accepte_val)
        )
        imported += 1

    conn.commit()
    return imported, {
        "col_id": col_id, "col_montant": col_montant,
        "col_delai": col_delai, "col_num": col_num, "col_accepte": col_accepte,
    }


# ── Import états d'avancement ─────────────────────────────────────────────────
def import_ea(source, sheet_name, conn,
              header_row=1, first_data_row=2,
              col_id=None, col_montant=None,
              overwrite=False):
    """
    Import états d'avancement.
